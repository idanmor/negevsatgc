...
//	                	if(i == 0 && j  < table.getColumns().size()){
//                    		text.append(table.getColumns().get(j).getText());
//                    		 text.append("\n");
//                    		//text.append(",");
//                    	}
	                    for(int k = 0 ; k < data[j].length ; k++){
	        ...
	        table.setOnMouseClicked(new EventHandler<MouseEvent>() {
	            ...
	            public void handle(MouseEvent t) {
	                if(t.getClickCount() == 2 && table.getSelectionModel().getSelectedItem() != null){
	                	createCompareStatisticsWindows(lineChart);
	                }
	                t.consume();
	            }
	        });
	    ...
	    private void createCompareStatisticsWindows(LineChart<String,Number> lineChart){
	    	lineChart.getData().clear();
